...
class AudioConfig(BaseModel):
    ...
    target_sample_rate: SampleRate = Field(default=SampleRate.speech, description="Target sample rate for transcription")
    flac_compression_level: CompressionLevel = Field(default=CompressionLevel.high, description="FLAC compression level")
    output_format: OutputFormat = Field(default=OutputFormat.flac, description="Output audio format")
    
    # Processing Settings
    silence_threshold: float = Field(default=0.01, ge=0.001, le=0.1, description="Amplitude threshold for silence detection")
    min_silence_duration: float = Field(default=25.0, ge=1.0, description="Minimum silence duration to split (seconds)")
    min_chunk_duration: float = Field(default=5.0, ge=1.0, description="Minimum chunk duration (seconds)")
    keep_silence: float = Field(default=1.0, ge=0.0, description="Silence to keep at boundaries (seconds)")
    chunk_duration: float = Field(default=30.0, ge=10.0, description="Processing chunk duration (seconds)")
...
def show_config_table(config: AudioConfig):
    ...
    table = Table(title="🔧 Audio Processing Configuration", show_header=True, header_style="bold cyan")
...
def interactive_config() -> AudioConfig:
    ...
    config.silence_threshold = FloatPrompt.ask(
        "Silence threshold (0.001=very sensitive, 0.1=less sensitive)",
        default=0.01
    )
    
    config.min_silence_duration = FloatPrompt.ask(
        "Minimum silence duration to create split (seconds)",
        default=25.0
    )
    ...
    if Confirm.ask("\nConfigure advanced settings?", default=False):
        ...
        config.keep_silence = FloatPrompt.ask(
            "Silence to keep at chunk boundaries (seconds)",
            default=1.0
        )
        
        config.chunk_duration = FloatPrompt.ask(
            "Processing chunk duration (seconds)",
            default=30.0
        )
        
        config.create_metadata = Confirm.ask(
    ...
    # Save option
    if Confirm.ask("\n💾 Save this configuration to file?", default=True):
...
def process(
    ...
    dry_run: bool = typer.Option(
        False, 
        "--dry-run", 
        help="Preview processing without actually converting files"
    ),
...
@app.command()
def config(
    show: bool = typer.Option(False, "--show", help="Show current default configuration"),
    create: bool = typer.Option(False, "--create", help="Create sample configuration file"),
    edit: bool = typer.Option(False, "--edit", help="Edit configuration interactively"),
    file: Optional[Path] = typer.Option(None, "--file", "-f", help="Configuration file to show/edit")
):
    """
...
def optimize(
    ...
    sample_rate: SampleRate = typer.Option(SampleRate.speech, "--sample-rate", "-r", help="Target sample rate"),
    compression: CompressionLevel = typer.Option(CompressionLevel.high, "--compression", "-c", help="FLAC compression level")
):
    """
...
def analyze(
    ...
    show_silence: bool = typer.Option(True, "--silence/--no-silence", help="Show silence segments"),
    threshold: float = typer.Option(0.01, "--threshold", "-t", help="Silence threshold")
):
    """
    ...
    try:
        ...
        if show_silence:
            console.print(f"\n🔇 [bold cyan]Silence Analysis[/bold cyan] (threshold: {threshold})")
            ...
            if silence_mask.any():
                ...
                for start, end in zip(silence_starts, silence_ends):
                    ...
                    if duration_seconds >= 25.0:  # Default min silence duration
                        long_silence_count += 1
                
                console.print(f"   Segments ≥25s: {long_silence_count}")
                
                if long_silence_count > 0:
                    console.print(f"   [green]✓ File would be split into {long_silence_count + 1} chunks[/green]")
                else:
                    console.print(f"   [yellow]! File would not be split (no 25s+ silence)[/yellow]")
            else:
                console.print(f"   [yellow]No silence detected at threshold {threshold}[/yellow]")
        ...
        console.print(f"   Current: {file_size_mb:.1f} MB → Optimized: ~{optimized_size:.1f} MB")
